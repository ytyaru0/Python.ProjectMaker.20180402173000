...
class ExtToLang:
    ...
    def __GetExt2LangTsvFile(self):
        for f in [pathlib.Path(__file__).parent / 'ext2lang.tsv']:
    ...
    def __GetFromTsv(self, path, ext):
        with open(path) as f:
            for line in f.read().split('\n'):
                cols = line.split('\t')
                if ext in cols[1:]: return cols[0]
    """
    # 応答1秒。yamlより早い
    def __GetFromTsv(self, path, ext):
        import csv
    ...
    def __GetTsvFile(self):
        for f in ['/tmp/work/Python.ProjectMaker.20180402173000/src/languages.tsv', self.__path_target]:
            if os.path.isfile(f): return f
    ...
    def __GetYamlFile(self):
        for f in ['/tmp/work/Python.ProjectMaker.20180402173000/src/languages.yml', self.__path_target]:
            if os.path.isfile(f): return f
